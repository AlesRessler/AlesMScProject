...
def spherical_deconvolution_fit(fibre_response_function, measurements, qhat, b_value, max_degree, number_of_samples_of_fibre_response_function, regularisation_strength=0.03):
    """
    ...
    measurements (np.array(1xM)): measured normalised (signals/b_0_signal) signals (where M is the number of measurements)
    ...
    design_matrix_pseudoinverse = np.linalg.pinv(design_matrix.T @ design_matrix + np.identity(number_of_spherical_harmonics_expansion_coefficients)*regularisation_strength) @ design_matrix.T
